...
class AmazonTransactions
{
	...
	public static function authorize(AmzPayments $amz_payments, $service, $order_ref, $amount, $currency_code = 'EUR', $timeout = 1440, $comment = '')
	{
		...
		if ($amz_payments->provocation == 'soft_decline' && $amz_payments->environment == 'SANDBOX')
		{
			...
			$authorize_request->setSellerAuthorizationNote(
					'{"SandboxSimulation": {"State":"Declined", "ReasonCode":"InvalidPaymentMethod", "PaymentMethodUpdateTimeInMins":2}}'
			);
		}
		...
		try
		{
			...
			$sql_arr = array('amz_tx_order_reference' => $order_ref,
							'amz_tx_type' => 'auth',
							'amz_tx_time' => time(),
	...
	public static function fastAuth(AmzPayments $amz_payments, $service, $order_ref, $amount, $currency_code = 'EUR', $comment = '')
	{
		...
		if (is_object($response))
		{
			if ($response->getAuthorizeResult()->getAuthorizationDetails()->getAuthorizationStatus()->getState() != 'Open')
				return $response;
			//$auth_id = $response->getAuthorizeResult()->getAuthorizationDetails()->getAmazonAuthorizationId();
	...
	{
		unset($comment);
		$sql_data_array = array('id_order' => $oid, 'id_order_state' => $status, 'date_add' => date('Y-m-d H:i:s'));
