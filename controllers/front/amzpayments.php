...
class AmzpaymentsAmzpaymentsModuleFrontController extends ModuleFrontController
{
	...
	public function __construct()
	{
		...
		//require_once (__DIR__ . '/../../override/classes/Address.php');
					...
					switch (Tools::getValue('method'))
					{
						...
						case 'getAddressBlockAndCarriersAndPayments':
							if ($this->context->customer->isLogged())
							{
								...
								$return = array_merge(array(
										...
										'HOOK_PAYMENT' => $this->_getPaymentMethods(),
										'no_address' => 0,
										'gift_price' => Tools::displayPrice(Tools::convertPrice(Product::getTaxCalculationMethod() == 1 ?
						...
						case 'updateAddressesSelected':
							...
							$physical_destination =
								$reference_details_result_wrapper->GetOrderReferenceDetailsResult->getOrderReferenceDetails()->getDestination()->getPhysicalDestination();
							...
							$address_delivery = Address::findByAmazonOrderReferenceIdOrNew(Tools::getValue('amazonOrderReferenceId'));
							...
							$address_delivery->amazon_order_reference_id = Tools::getValue('amazonOrderReferenceId');
							...
							//$addresses = $this->context->cart->getAddressCollection();
						...
						case 'executeOrder':
							...
							if (Tools::getValue('confirm'))
							{
								...
								if (!AmazonTransactions::isAlreadyConfirmedOrder(Tools::getValue('amazonOrderReferenceId')))
								{
									...
									$set_order_reference_details_request->getOrderReferenceAttributes()->setSellerOrderAttributes(
											new OffAmazonPaymentsService_Model_SellerOrderAttributes()
									);
									$set_order_reference_details_request->getOrderReferenceAttributes()->getSellerOrderAttributes()->setSellerOrderId(
										self::$amz_payments->createUniqueOrderId((int)$this->context->cart->id)
									);
									$set_order_reference_details_request->getOrderReferenceAttributes()->getSellerOrderAttributes()->setStoreName(
									...
									$sql_arr = array(
											...
											'amz_tx_status'=>$reference_details_result_wrapper->GetOrderReferenceDetailsResult->getOrderReferenceDetails()->getOrderReferenceStatus()->getState(),
											...
											'amz_tx_amount'=>$reference_details_result_wrapper->GetOrderReferenceDetailsResult->getOrderReferenceDetails()->getOrderTotal()->getAmount()
									);
								...
								{
									$s_street = Tools::substr($physical_destination->getAddressLine3(), 0, strrpos($physical_destination->getAddressLine3(), ' '));
									$s_street_nr = Tools::substr($physical_destination->getAddressLine3(), strrpos($physical_destination->getAddressLine3(), ' ') + 1);
									$s_company_name = trim($physical_destination->getAddressLine1().$physical_destination->getAddressLine2());
								}
								else if ((string)$physical_destination->getAddressLine2() != '')
								{
									$s_street = Tools::substr($physical_destination->getAddressLine2(), 0, strrpos($physical_destination->getAddressLine2(), ' '));
									$s_street_nr = Tools::substr($physical_destination->getAddressLine2(), strrpos($physical_destination->getAddressLine2(), ' ') + 1);
									$s_company_name = trim($physical_destination->getAddressLine1());
								}
								else
								{
									$s_street = Tools::substr($physical_destination->getAddressLine1(), 0, strrpos($physical_destination->getAddressLine1(), ' '));
									$s_street_nr = Tools::substr($physical_destination->getAddressLine1(), strrpos($physical_destination->getAddressLine1(), ' ') + 1);
								}
									...
									if (is_object($authorization_response_wrapper))
									{
										...
										if ($status == 'Declined')
										{
											...
											if ($reason == 'InvalidPaymentMethod')
											{
												die(Tools::jsonEncode(array(
														'hasError' => true,
														'errors' => array(Tools::displayError(self::$amz_payments->l('Die von Ihnen gewählte Zahlungsart ist leider momentan nicht verfügbar. Bitte wählen Sie eine andere..')))
												)));
											}
											else
											{
												die(Tools::jsonEncode(array(
														...
														'errors' => array(Tools::displayError(self::$amz_payments->l('Your selected payment method has been declined. Please chose another one.')))
												)));
											}
										}
										...
										if (self::$amz_payments->capture_mode == 'after_auth')
											$amazon_capture_id = AmazonTransactions::capture(self::$amz_payments, $this->service, Tools::getValue('amazonOrderReferenceId'), $total, $currency_code);
									}
								}
								...
								if (self::$amz_payments->authorization_mode == 'after_checkout')
								{
									...
									if (self::$amz_payments->capture_mode == 'after_auth' && isset($amazon_authorization_id) && $amazon_authorization_id !== false && $amazon_authorization_id != null)
										$amazon_capture_id = AmazonTransactions::capture(self::$amz_payments, $this->service, $amazon_authorization_id, $total, $currency_code);
								}
	...
	public function initContent()
	{
		...
		foreach ($this->context->cart->getCartRules() as $rule)
			...
			if (isset($this->context->cookie->amz_access_token) && 
				...
				trim($this->context->customer->amazon_customer_id) == '')
