...
class AmzpaymentsUser_To_ShopModuleFrontController extends ModuleFrontController
{
	...
	public function __construct()
	{
		...
		//require_once (__DIR__ . '/../../override/classes/Customer.php');
			...
			if (Tools::isSubmit('method'))
			{
				switch (Tools::getValue('method'))
				{
					...
					case 'setusertoshop' :
						...
						curl_setopt($c, CURLOPT_HTTPHEADER, array('Authorization: bearer '.$accessTokenValue));
						curl_setopt($c, CURLOPT_RETURNTRANSFER, true);
						curl_setopt($c, CURLOPT_CAINFO, self::$amz_payments->ca_bundle_file);
						$r = curl_exec($c);
						curl_close($c);
						...
						if ($customers_local_id = Customer::findByAmazonCustomerId($customer_userid))
						{

							// Customer already exists - login
							Hook::exec('actionBeforeAuthentication');
							$customer = new Customer();
							$authentication = $customer->getByCustomerID($customers_local_id);
						...
						{
							if (Customer::findByEmailAddress($customer_email))
							{
							...
							else
							{
								...
								$customer->amazon_customer_id = $customer_userid;
	...
	protected function sendConfirmationMail(Customer $customer)
	{
		...
		return Mail::Send($this->context->language->id, 'account',
							Mail::l('Welcome!'),
